...
class Client(base.Client):
    ...
    def _load_data(self) -> None:
        ...
        if (
            ...
            logging.info(
                "[%s] Dataset size: %s", self, self.datasource.num_train_examples(self.client_id)
            )

    def _allocate_data(self) -> None:
        """Allocate training or testing dataset of this client."""

        X_train,y_train,X_test,y_test = self.datasource.get_dataset(self.client_id)
    ...
    def _load_payload(self, server_payload) -> None:
        ...
        logging.info("[%s] loading payload", self)
    ...
    async def _train(self):
        ...
        try:
            ...
            weights = self.trainer.train(self.trainset, self.client_id, self.sampler)
        ...
        if (hasattr(Config().clients, "do_test") and Config().clients.do_test) and (
            ...
            accuracy = self.trainer.testClient(self.testset, self.trainset, weights, self.testset_sampler)
            ...
            if hasattr(Config().trainer, "target_perplexity"):
                ...
                logging.info("[%s] Test CI: %.2f%%", self, 100 * accuracy[0])
                logging.info("[%s] Test IBS: %.2f%%", self, 100 * accuracy[1])
            path = (
            ...
            weights = self.trainer.pickBestTrees(weights, self.trainset, self.testset, int(Config.server.n_trees/Config.clients.total_clients))
        else:
            accuracy = 0
        ...
        if (
            ...
            sleep_seconds = 1#Config().client_sleep_times[self.client_id - 1]
            avg_training_time = 20#Config().clients.avg_training_time
